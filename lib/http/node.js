...
TableStore.NodeHttpClient = TableStore.util.inherit({
  handleRequest: function handleRequest(httpRequest, httpOptions, callback, errCallback) {
    ...
    if(httpRequest.proxyUrl){
      endpoint = parseURL(httpRequest.proxyUrl)
      httpRequest.headers['real-endpoint']= httpRequest.endpoint.href.replace(/\/*$/,'')+ httpRequest.path;
      httpRequest.headers['real-method']= httpRequest.method 
      httpRequest.method = 'POST'
      httpRequest.headers['Host'] = endpoint.hostname
    }
    /*****************************/
